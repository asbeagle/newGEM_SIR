...
## (Technically, 1-ECDF!)
## R0 = 1 plots
var1=c('c',   'alpha','c',    'c',    'shed', 'shed')
var2=c('shed','gamma','gamma','alpha','gamma','alpha')
i <- 1
...
ggplot(data2, aes(x=size, y=ECDF, group=Variance, colour=Variance)) + 
  ...
  ggtitle("Intragroup trait pairings                                            Intergroup trait pairings") + 
  theme_bw() + 
...
data %>% mutate(prev=I/(S+I+R)) %>% # compute prevalence
  group_by(Variance, cov, traits, rep) %>% # calculate the maximum prevalence for each simulation
  summarise(maxPrev=max(prev), maxT=max(t)) %>% 
  filter(maxT < 100) %>% # look only at epidemics that faded out
  group_by(Variance, cov, traits) %>% # summarize across replicates
  summarize(prop=sum(maxPrev > 0.2)/100) -> fadeout_data # compute the fraction of replicates with a prevalence above any particular percentage
...
png(filename="./All_peak_against_dispersion_R0=4.png", height=2.5, width=9, units='in', res=300)
