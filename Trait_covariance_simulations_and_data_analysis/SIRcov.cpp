...
List SIRcovCS(NumericVector params, IntegerVector x, double tmax) {
  ...
  while(t < tmax && I > 0) {
    ...
    idVec = storage(_,0); // all IDs
    cVec = storage(_,1); // all c values
    sVec = storage(_,2); // all s values
    // get the IDs, c, and s values for only the living individuals
    idAlive = idVec[!is_na(storage(_,4))];
    cAlive = cVec[!is_na(storage(_,4))];
    sAlive = sVec[!is_na(storage(_,4))];
