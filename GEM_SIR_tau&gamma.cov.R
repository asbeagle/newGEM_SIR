...
gillespie.SIR.cov_taugamma <- function(tmax, params, corr, x, seed=floor(runif(1,1,1e5))) {
  ...
  results[[1]] <- list(t, S, tau_i, gamma_i, R)
  ...
  while(t < tmax & length(tau_i) >0 ){ 
    ...
    if (event%in%1:length(tau_i)){### infection
      ...
      tau_i <- c(tau_i, (infection[,1]^2)/((h^2)+(infection[,1]^2))) # add to list of tau i
      gamma_i <- c(gamma_i, infection[,2]) # add to list of gamma i
    }
    else if(event%in%((length(tau_i)+1):(length(tau_i)+length(gamma_i)))){ # recovery
      R <- R+1
      tau_i <- tau_i[-(event-length(gamma_i))]
      gamma_i<- gamma_i[-(event-length(gamma_i))]
    }
    else if(event==(2*(length(tau_i)))+1){ # death of I
      ind=sample(1:length(tau_i),1)
      tau_i<-tau_i[-(ind)] 
      gamma_i<-gamma_i[-(ind)]
    } 
    
    else if (event==(2*(length(tau_i)))+2){ ### birth
      S <- S+1
    }
    else if (event==(2*(length(tau_i)))+3) { ### death of S
      S <- S-1
    }
    else    {   ### death of R
      R <- R-1
    ...
    results[[i]] <- list(t, S, tau_i, gamma_i , R)
    i <- i +1
...
all.new.params2 = c(c=.2, shed=.2, sd_s=.01, sd_g=.01, h=.1, alpha=.01, gamma=.3, b=2.5, d=.4, bs=.01)
