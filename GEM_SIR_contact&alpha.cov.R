...
cov_parms = c(c=.1, shed=.05, sd_c=.1, sd_a=.32, h=.1, alpha=.1, gamma=.1, b=2.5, d=.1, bs=.01)
corr <- matrix(c(1,0,0,1), nrow=2, byrow=T)
 
 
cov_out <- gillespie.SIR.cov_calpha(tmax, cov_parms, corr, initial_state)
 
par(mfrow=c(1,1))
plot.ts(cov_out[,2], col="blue", ylim=c(-5, 250))
lines(cov_out[,3], col="red")
lines(cov_out[,4], col="green")
