...
gillespie.SIR.cov_cshed <- function(tmax, params, corr, x, seed=floor(runif(1,1,1e5))) {
  set.seed(seed)
  c = params["c"]
  shed = params["shed"]
  alpha = params["alpha"]
  ...
  results[[1]] <- list(t, S, c_i, shed_i, R)
  ...
  while(t < tmax & length(c_i) >0 ){ 
    ...
    results[[i]] <- list(t, S,shed_i, c_i , R)
    i <- i +1
