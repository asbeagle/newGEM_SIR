### gillespie model with stratified gamma variation
...
gillespie.SIR.strat.varA <- function(tmax, params, x, seed=floor(runif(1,1,1e5))) {
  ...
  alpha_values<-c(.15, .25, .35)
  alpha_i<-sample(alpha_values, 10, replace=TRUE)
...
x = c(S=70, I=10, R=0)
tmax <- 150
params4 = c(beta=.25, alpha=.01, gamma=.15, varG=1e-3, b=2, d=0.4, bs=0.01, ds=0.01)

out14 <- gillespie.SIR.strat.varA(tmax, baselineparams, x)
