...
alpha_gamma_data <-rbind(sumdata_use1,sumdata_use2,sumdata_use3,
                        sumdata_use4,sumdata_use5,sumdata_use6,
                        sumdata_use7,sumdata_use8,sumdata_use9)

frac0_alpha_gamma <- rbind(sumdata_use1,sumdata_use2,sumdata_use3,sumdata_use4,sumdata_use5,sumdata_use6,sumdata_use7,sumdata_use8,sumdata_use9)
...
# plot
alpha_gamma<-ggplot(data=frac0_alpha_gamma, mapping = aes(x=Corr, y = frac0Reff))+
  geom_point(data = frac0_alpha_gamma, aes(color=Var, shape = Var), alpha = .7, size = 3.5)+
  scale_color_manual(values=c("darkblue", "darkgreen", "pink"))+
  labs(title = "Alpha-Gamma", y = "% no secondary infections")+
  scale_x_discrete(limits=c("no", "neg","pos"))

alpha_gamma<- ggplot(alpha_gamma_data, mapping = aes(x=frac0Reff, y=R))+
...
ggarrange(contact_shed, contact_alpha, contact_gamma,  alpha_gamma, shed_alpha, shed_gamma, nrow=2, ncol=3, common.legend = TRUE, legend="right")
