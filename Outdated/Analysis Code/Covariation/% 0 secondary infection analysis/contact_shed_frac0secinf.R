### fraction 0 secondary infection
# shed shed
...
frac0_contact_shed <- rbind(sumdata_use1,sumdata_use2,sumdata_use3,sumdata_use4,sumdata_use5,sumdata_use6,sumdata_use7,sumdata_use8,sumdata_use9)

contact_shed_data <-rbind(sumdata_use1,sumdata_use2,sumdata_use3,
...
contact_shed<-ggplot(data=frac0_contact_shed, mapping = aes(x=Corr, y = frac0Reff))+
  geom_point(data = frac0_contact_shed, aes(color=Var, shape = Var), alpha = .7, size = 3.5)+
  scale_color_manual(values=c("darkblue", "darkgreen", "pink"))+
  labs(title = "Contact-Shed", y = "% no secondary infections")+
  scale_x_discrete(limits=c("no", "neg","pos"))

contact_shed<- ggplot(contact_shed_data, mapping = aes(x=frac0Reff, y=R))+
