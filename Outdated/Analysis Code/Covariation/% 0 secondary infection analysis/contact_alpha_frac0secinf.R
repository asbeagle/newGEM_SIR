...
(lapply(get(dataName), function(l) l[[2]]$numInf)) %>% unlist
...
contact_alpha_data <-rbind(sumdata_use1,sumdata_use2,sumdata_use3,
                           sumdata_use4,sumdata_use5,sumdata_use6,
                           sumdata_use7,sumdata_use8,sumdata_use9)

frac0_c_alpha <- rbind(sumdata_use1,sumdata_use2,sumdata_use3,sumdata_use4,sumdata_use5,sumdata_use6,sumdata_use7,sumdata_use8,sumdata_use9)

# plot
contact_alpha<-ggplot(data=frac0_c_alpha, mapping = aes(x=Corr, y = frac0Reff))+
  geom_point(data = frac0_c_alpha, aes(color=Var, shape = Var), alpha = .7, size = 3.5)+
  scale_color_manual(values=c("darkblue", "darkgreen", "pink"))+
  labs(title = "Contact-Alpha", y = "% no secondary infections")+
  scale_x_discrete(limits=c("no", "neg","pos"))

contact_alpha

contact_alpha<- ggplot(contact_alpha_data, mapping = aes(x=frac0Reff, y=R))+
