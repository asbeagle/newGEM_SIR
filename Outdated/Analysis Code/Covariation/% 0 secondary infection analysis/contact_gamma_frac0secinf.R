...
frac0_c_gamma <- rbind(sumdata_use1,sumdata_use2,sumdata_use3,sumdata_use4,sumdata_use5,sumdata_use6,sumdata_use7,sumdata_use8,sumdata_use9)

contact_gamma_data <-rbind(sumdata_use1,sumdata_use2,sumdata_use3,
                            sumdata_use4,sumdata_use5,sumdata_use6,
                            sumdata_use7,sumdata_use8,sumdata_use9)


# plot
contact_gamma<- ggplot(data=frac0_c_gamma, mapping = aes(x=Corr, y = frac0Reff))+
  geom_point(data = frac0_c_gamma, aes(color=Var, shape = Var), alpha = .7, size = 3.5)+
  scale_color_manual(values=c("darkblue", "darkgreen", "pink"))+
  labs(title = "Contact-Gamma", y = "% no secondary infections")+
  scale_x_discrete(limits=c("no", "neg","pos"))

contact_gamma<- ggplot(contact_gamma_data, mapping = aes(x=frac0Reff, y=R))+
...
ggarrange(contact_alpha, contact_gamma, nrow=1, ncol=2, common.legend = TRUE, legend="right")
