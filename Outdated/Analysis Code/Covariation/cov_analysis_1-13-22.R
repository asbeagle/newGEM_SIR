  ...
  for (covMatrix in c("nocorr", "negcorr", "poscorr")) {
    for (varLevel in c("hivar","medvar","lowvar")) {
      ...
      data.frame(traits=rep(paste(var1[j],var2[j],sep='-'),50),
                 ...
                 meantop20 = unlist(lapply(out, function(o) mean(sum(sort(o[[2]]$numInf)[round(0.8*nrow(o[[2]])):nrow(o[[2]])])/sum(o[[2]]$numInf)))),
                 meanI=unlist(lapply(out, function(o) mean(tail(o[[1]]$I,n=20)))),
...
#####
data<-subset(data, meanI>10)
ggplot(data, aes(x=meantop20,y=meanI, shape=cov, colour=var)) + 
  geom_point() + 
  facet_wrap(~traits, scales = "free")+
  labs(title="Med R0", x="Mean prop. of infections caused by SS", y="Final Epidemic Size")+
...
ggplot(data, aes(x=meantop20,y=maxI, shape=cov, colour=var)) + 
  geom_point() + 
  facet_wrap(~traits, scales = "free")+
  labs(title="Med R0", x="Mean prop. of infections caused by SS", y="Peak Epidemic Size")+
  scale_color_manual(values=c("pink", "darkblue", "darkgreen"))+
  theme_bw()+
  ylim(100, 210)+
  xlim(.6, 1)+
...
ggplot(subset(data, traits%in%c("alpha-gamma","c-shed", "shed-alpha", "shed-gamma")),aes(x=var,y=maxI,shape=cov,colour=cov)) + 
  geom_boxplot() + 
  facet_wrap(~traits, scales = "free")+
  labs(title="Med R0")+
...
### to set the sizes of axis title and labels separately
...
intergroup<-ggplot(subset(data, traits%in%c("c-alpha","c-gamma")), aes(x=t, y=I, group=rep)) +
...
ggarrange(intragroup, intergroup, legend = "right", common.legend = TRUE, nrow=2, ncol=1,labels = "Med R0",hjust=-8, vjust=2.25,
