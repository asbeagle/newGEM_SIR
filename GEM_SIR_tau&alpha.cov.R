...
gillespie.SIR.cov_taualpha <- function(tmax, params, corr, x, seed=floor(runif(1,1,1e5))) {
  set.seed(seed)
  c = params["c"]
  shed = params["shed"]
  h = params["h"]
  ...
  results[[1]] <- list(t, S, tau_i, alpha_i, R)
  ...
  while(t < tmax & length(tau_i) >0 ){ 
    ...
    else if(event%in%((length(tau_i)+1):(length(tau_i)+(length(alpha_i))))){ # death of I
      tau_i <- tau_i[-(event-length(alpha_i))]
      alpha_i<- alpha_i[-(event-length(alpha_i))]
    } 
    
    else if(event==(2*length(tau_i))+1){ # recovery
      R <- R+1
    ...
    results[[i]] <- list(t, S,tau_i, alpha_i , R)
    i <- i +1
